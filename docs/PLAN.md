...
- ~250 steps (~2 epochs over the pool). Checkpoints (adapter + optimizer state + scheduler + RNG, ~1 GB each) every **25 steps** (`--save-steps 25`, decided 2026-09-16), so 10 per run including the midpoint (125) and end (250; 500 if extended). Step 0 is the base model with no adapter.
- Resume policy: deliberate pauses are handled by stopping the pod and relaunching from scratch (a resumed run is not bit-identical to an uninterrupted one because rollout sampling continues from a different RNG state). Checkpoint-resume is for crash recovery only (pod failure or late OOM) and must be recorded as "resumed at step N" in the run metadata and report. The 500-step extension resumes from step 250. The baseline GPT-5 judge pass was interrupted and resumed on 2026-09-16, but judge calls are independent per response, so that has no analogous effect.
...
- **Stage 1 (GPU, Qwen2.5-7B-Instruct, one merged pipeline test, ~30–45 min ≈ $2–3)**
  ...
  6. Go/no-go passed (2026-09-13). Spanish-arm launch procedure (decided 2026-09-16; exact commands in `STAGE1_REPORT.md` §9): start pod → sync + bootstrap → screening 4,000 × 8 → training under tmux with `--save-steps 25 --report-to none`, `&&`-chained into midpoint/final math and StrongREJECT generation → first health poll at step 10–15 (filtered one-liner) → **pause point for this first launch**: kill the tmux session and stop the pod; cost analysis with nothing running; relaunch from step 0 on confirmation (screening pool persists). Subsequent polls every 30–45 min. After completion: pull outputs, GPT-5 judge on the final (`judge_api.py --select all`), `report.py --safety-judge api`, stop pod.
