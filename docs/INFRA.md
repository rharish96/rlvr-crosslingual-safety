...
- Session log:
  ...
  - 2026-09-19/20 English control on pod `117rcbsl79ii1k` (restarted fine; SSH `205.196.19.19:9024`): 20:46Z start → 20:48Z launch → 02:25Z `ARM_DONE` → `base_en` eval → 02:31Z stop. 20,596 s = 5.72 h ≈ **$20.0**. OpenAI `en_final` judge $5.74 (OpenAI total ≈ $23.3).
  - 2026-09-18/19 Spanish arm, full run on new pod `117rcbsl79ii1k` (SSH `205.196.19.19:8967`): 22:37Z create → 22:39Z launch → ~04:00Z `ARM_DONE` → 04:03Z stop. 19,512 s = 5.42 h, **$18.98 posted** (GPU $18.95 + disk $0.03). Adapters (9.6 GB) and outputs on the volume; outputs pulled to the Mac. See `docs/ES_ARM_REPORT.md`.
