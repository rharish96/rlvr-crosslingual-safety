...
- **English control launched 2026-09-19 20:48Z** on pod `117rcbsl79ii1k` (restarted successfully this time; SSH `205.196.19.19:9024`), `launch_arm.sh en` in tmux `en`, log `/workspace/logs/en_seed0.log`, marker `en_seed0.DONE`. Expected done ≈ 02:10Z on 09-20. After `ARM_DONE` and before stopping the pod: run the missing English math baseline, `uv run python scripts/eval_math.py --lang en --model 7b --k 8 --tag base_en --out-dir /workspace/outputs` (the safety baseline is shared). Then pull, gate, stop, `judge_api.py --tag en_final`, `report.py --arm en --safety-judge api --math-base outputs/eval_math/base_en.json …`.
